...
void Generator::update()
{
  int previousReadouts = m_trajectory ? m_trajectory->numReadouts : 0;
	...
	{
		case Spiral:
      m_trajectory = generateSpirals(m_variableDensity, m_fieldOfView[0], m_spatialResolution[0], m_readoutDuration, 1, m_samplingInterval, m_readouts, Archimedean, 0, m_fieldOfView[0], m_gradientLimit, m_slewRateLimit);
			break;
		case Radial:
		...
		case Cones3D:
		{
			...
			if(m_cones)
        deleteCones(&m_cones);
      m_cones = generateCones(m_fieldOfView[0], m_fieldOfView[2], m_variableDensity, m_spatialResolution[0], m_spatialResolution[2], 48, 1, NoCompensation, m_readoutDuration, m_samplingInterval, fieldOfView, m_gradientLimit, m_slewRateLimit, STORE_ALL);
